SELECT * FROM ACCOUNT;

Insert into ACCOUNT (ACC_NO,ACC_ID,ACC_PW,ACC_NAME,ACC_BIRTH,ACC_EMAIL,ACC_PHONE,ACC_ZIPCODE,ACC_ADDR,ACC_ADDR2,ACC_ISREG,ACC_POINT,ACC_M_C) values (1,'m1','321321','독고수지','20000719','m1@naver.com','01012345678','06220','서울특별시 강남구 역삼동 717','한국은행 강남본부','Y',300000,'M');

SELECT * FROM WISH JOIN PRODUCT USING(PROD_NO) WHERE ACC_NO=100;

SELECT * FROM QNA;

SELECT * FROM CART;

SELECT * FROM ACCOUNT;

SELECT * FROM CART JOIN PROD_OPTION USING(PROD_ID) JOIN PRODUCT USING(PROD_NO) WHERE ACC_NO=100;

SELECT * FROM PRODUCT;

UPDATE PRODUCT SET PROD_FRONT_IMG='f_img';
