...
%>
    ...
    <% List<Vo_Order> prod_order = (List)request.getAttribute("olist");%>
...
<meta charset="utf-8">
  ...
  <%@ include file="/header.jsp"%>
	...
	<div class="orderInfo" style = "width: 70%;">
        ...
        <ul>
			<li>주문번호 : <strong>${Onum.order_no}</strong>
			</li>
            <li>주문일자 : <span>${Onum.order_date}</span>
			</li>
        </ul>
	...
	<div class="orderArea">
		...
		<th scope="row">최종결제금액</th>
			...
			<c:set var = "sum" value = "${sum + (olist.order_price * olist.order_amount)}"></c:set>
			</c:forEach>
           <td>
             <strong class="txtEm txt18" id = "aftertotal">
           	<fmt:formatNumber>${sum}</fmt:formatNumber>
            </strong>
	...
	<div class="orderListArea">
		...
		<tr>
			...
			<td colspan="7">
				...
				<span class="displaynone"></span>
				 + 배송비 0 <span class="displaynone"> - 상품할인금액&nbsp;${Onum.order_point}</span> = 합계 : 
				 <strong class="txtEm gIndent10">
				 
		 		<span class="txt18"><fmt:formatNumber>${sum - Onum.order_point}</fmt:formatNumber></span>원
		 		</strong> 
		 		<span class="displaynone"></span>
			</td>
		</tr>
		...
		<tbody class="xans-element- xans-order xans-order-normalresultlist center">
			...
			<td class="thumb">
				<a href="<%=request.getContextPath()%>/Product.do?command=ProdDetail&pseq=${olist.prod_no}&catdno=${olist.catd_no}">
            ...
            <td class="right">
              ...
              <strong><fmt:formatNumber>${olist.order_price}</fmt:formatNumber>원</strong>
              <div class="displaynone">
              </div>
              </div>
           	</td>
                        
            <td><fmt:formatNumber>${olist.order_amount}</fmt:formatNumber></td>
            <td><span class="txtInfo">-</span></td>
            <td><div class="txtInfo">기본배송</div></td>
            <td class="right">
			<strong><fmt:formatNumber>${olist.order_amount * olist.order_price}</fmt:formatNumber></strong>
	...
	<tbody class="center">
		...
		<strong><span class="txt23"><fmt:formatNumber>${sum}</fmt:formatNumber></span>원</strong><span class="displaynone"></span>
		</div>
		</td>
		<td class=""><div class="box txt16">
		<strong>- <span class="txt23"><fmt:formatNumber>${Onum.order_point}</fmt:formatNumber></span>원</strong><span class="displaynone"></span>
		</div>
		</td>
		...
		<strong>
		<span class="txt23">
		<fmt:formatNumber>${sum-Onum.order_point}</fmt:formatNumber>
		</span>원
		</strong>
		<span class="displaynone"></span>
	</div>
	...
	<th scope="row"><strong>적립금 적용</strong></th>
        <td><strong class="txt14"><fmt:formatNumber>${Onum.order_point}</fmt:formatNumber></strong>원</td>
    </tr>
      ...
      <div class="ec-base-table gMerge displaynone">
		...
		<th scope="row">우편번호</th>
	      <td><span>${Onum.order_zipcode}</span></td>
	    </tr>
		<tr class="">
		<th scope="row">주소</th>
          <td><span>${Onum.order_addr}&nbsp;${Onum.order_addr2}</span></td>
         </tr>
		<tr>
